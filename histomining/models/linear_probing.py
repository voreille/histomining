...
class LinearProbingBase(pl.LightningModule):
    def __init__(
        self, feature_dim, num_classes, class_weights=None, backbone=None, lr=1e-3, weight_decay=0
    ):
        super().__init__()
    ...
    def test_step(self, batch, batch_idx):
        ...
        outputs = self(inputs)
        loss = self.criterion(outputs, labels)
        accuracy = MulticlassAccuracy(num_classes=self.num_classes).to(inputs.device)(
            outputs, labels
        )
